...
using Hoyer.Champions.Taya.Modes;
using UnityEngine;
...
namespace Hoyer.Champions.Taya
{
    public class Main : IAddon
    {
        ...
        {
            if (Game.IsInGame && LocalPlayer.Instance.CharName == "Taya") LocalPlayer.EditAimPosition = false;
        }

        private void SpellInit()
        {
            if (LocalPlayer.Instance.CharName != "Taya") return;
            ActiveSkills.Active.Add(new SkillBase(AbilitySlot.Ability1, SkillType.Line, 7.9f, 17, 0.3f));
            ActiveSkills.ActiveInfos.AddRange(AbilityDatabase.Get("Taya", AbilitySlot.Ability1));
            ActiveSkills.Active.Add(new SkillBase(AbilitySlot.Ability2, SkillType.Line, 10.25f, 26.5f, 0.3f));
            ActiveSkills.ActiveInfos.AddRange(AbilityDatabase.Get("Taya", AbilitySlot.Ability2));
            ActiveSkills.Active.Add(new SkillBase(AbilitySlot.Ability3, SkillType.Line, 7.8f, 13.5f, 0.3f));
            ActiveSkills.ActiveInfos.AddRange(AbilityDatabase.Get("Taya", AbilitySlot.Ability3));
            ActiveSkills.Active.Add(new SkillBase(AbilitySlot.Ability4, SkillType.Circle, 10, 0, 2, 0.5f));
            ActiveSkills.ActiveInfos.AddRange(AbilityDatabase.Get("Taya", AbilitySlot.Ability4));
            ActiveSkills.Active.Add(new SkillBase(AbilitySlot.Ability5, SkillType.Circle, 6.8f, 12, 1));
            ActiveSkills.ActiveInfos.AddRange(AbilityDatabase.Get("Taya", AbilitySlot.Ability5));
            ActiveSkills.Active.Add(new SkillBase(AbilitySlot.Ability7, SkillType.Line, 7.8f, 13.5f, 0.3f));
            ActiveSkills.ActiveInfos.AddRange(AbilityDatabase.Get("Taya", AbilitySlot.Ability7));
            ActiveSkills.Active.Add(new SkillBase(AbilitySlot.EXAbility1, SkillType.Line, 10.5f, 24.5f, 0.3f));
            ActiveSkills.ActiveInfos.AddRange(AbilityDatabase.Get("Taya", AbilitySlot.EXAbility1));
            ActiveSkills.Active.Add(new SkillBase(AbilitySlot.EXAbility2, SkillType.Line, 8.8f, 26.5f, 0.3f));
            ActiveSkills.ActiveInfos.AddRange(AbilityDatabase.Get("Taya", AbilitySlot.EXAbility2));
        }

        private void OnUpdate(EventArgs args)
        {
            if (!Enabled || !Game.IsInGame || Game.CurrentMatchState != MatchState.InRound || LocalPlayer.Instance.CharName != "Taya")
            {
                DebugOutput = "";
                return;
            }
        ...
        {
            Console.WriteLine("Unload Taya Started");
            MenuHandler.Unload();
            MenuEvents.Initialize -= MenuHandler.Init;
            MenuEvents.Update -= MenuHandler.Update;
            ActiveSkills.Initialize -= SpellInit;
            ...
            Game.OnUpdate -= OnUpdate;
            Game.OnPreUpdate -= Game_OnDraw;
            Console.WriteLine("Unload Taya Ended");
        }
    }
}