...
namespace PipPestilus
{
    public class PipPestilus : IAddon
    {
        ...
        private static readonly string HeroName = "Pestilus";
        ...
        private const float EX2Speed = 27f;
        private const float FSpeed = 27f;
        ...
        private const float FRadius = 0.45f;
        private const float EX1Radius = 0.35f;
        private const float EX2Radius = 0.4f;
        ...
        private static void InitMenu()
        {
            ...
            KeysMenu.Add(new MenuKeybind("keys.Space", "AFK Space", UnityEngine.KeyCode.Space));
            KeysMenu.Add(new MenuKeybind("keys.R", "AFK R", UnityEngine.KeyCode.R));
            ...
            HealMenu.Add(new MenuCheckBox("heal.useM1", "Heal with M1 (Moucherons samer)", true));
            HealMenu.Add(new MenuSlider("heal.useM1.safeRange", "Target ally safe range", 4f, 10f, 0f));
            ...
            HealMenu.Add(new MenuCheckBox("heal.useM.fullHP", "    ^ Use even if target ally has full health", true));
        ...
        private static void OnUpdate(EventArgs args)
        {
            ...
            else if ((KeysMenu.GetKeybind("keys.Space")) || (KeysMenu.GetKeybind("keys.R")))
            {
        ...
        private static void ComboMode()
        {
            ...
            x.HasBuff("Weaken") || x.HasBuff("Incapacitate") ||
            x.HasBuff("DeadlyInjectionBuff") || x.HasBuff("PhantomCutBuff") || x.HasBuff("EntanglingRootsBuff") || x.HasBuff("ArmorBreak") ||
            x.HasBuff("Frozen") || x.HasBuff("StormStruckDebuff") || x.HasBuff("BrainBugDebuff") ||
            // x.HasBuff("HandOfCorruptionBuff) ||
            x.HasBuff("SheepTrickDebuff") || x.HasBuff("Stun") || x.HasBuff("Silence"));
            //
            ...
            var targetM2 = TargetSelector.GetTarget(enemiesToTargetProjs, targetMode, M1Range);
            ...
            if (isCastingOrChanneling)
            {
                ...
                switch (LastAbilityFired)
                {
                    ...
                    if (alliesQ != null)
                    {
                        LocalPlayer.Aim(alliesQ.MapObject.Position);
                    }
                    break;
                    ...
                    case AbilitySlot.EXAbility1:
                        if (targetEX1 != null)
                        {
                            LocalPlayer.Aim(targetEX1.MapObject.Position);
                        }
                        break;
            ...
            if (ComboMenu.GetBoolean("combo.useM1") && MiscUtils.CanCast(AbilitySlot.Ability2))
            {
        ...
        private static void HealOthers()
        {
            ...
            var useM1 = HealMenu.GetBoolean("heal.useM1");
            var useM2 = HealMenu.GetBoolean("heal.useM2");
            var useMFullHP = HealMenu.GetBoolean("heal.useM.fullHP");
            ...
            var possibleAlliesM2 = EntitiesManager.LocalTeam.Where(x => !x.IsLocalPlayer && 
            ...
            if (!useMFullHP)
            {
                ...
                possibleAlliesM2 = possibleAllies.Where(x => x.Living.Health < (x.Living.MaxRecoveryHealth - 20f));
            }

            var allyM1 = TargetSelector.GetTarget(possibleAllies, TargetingMode.NearMouse, M1Range);
            var allyM2 = TargetSelector.GetTarget(possibleAlliesM2, TargetingMode.NearMouse, M2Range);
            ...
            if (isCastingOrChanneling)
            {
                ...
                switch (LastAbilityFired)
                {
                    ...
                    case AbilitySlot.Ability1:
                        if (allyM1 != null)
                        {
                            var pred = TestPrediction.GetNormalLinePrediction(MyPos, allyM1, M1Range, M1Speed, M1Radius, true);
                            if (pred.CanHit)
                            {
                                LocalPlayer.Aim(pred.CastPosition);
                            }
            ...
            if (useM1 && MiscUtils.CanCast(AbilitySlot.Ability1))
            {
                if (LastAbilityFired == null && allyM1 != null && (LucieHero.EnemiesAroundAlive(M2safeRange) < 1))
                {
                    var pred = TestPrediction.GetNormalLinePrediction(MyPos, allyM1, M1Range, M1Speed, M1Radius, true);
                    if (pred.CanHit)
                    {
        ...
        private static void HealSelf()
        {
            ...
            var shouldM2 = useM2 && (useMFullHP || LucieHero.Living.Health < (LucieHero.Living.MaxRecoveryHealth - 6f));
