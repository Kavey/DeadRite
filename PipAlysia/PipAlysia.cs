...
namespace PipAlysia
{
    public class PipAlysia : IAddon
    {
        ...
        private static readonly string HeroName = "Alysia";
        ...
        private const float M2Radius = 0.3f;
        private const float QRadius = 2f;
        private const float ERadius = 2.25f;
        ...
        {
            LucieMenu = new Menu("pipluciemenu", "DaPip's Alysia");
            ...
            ComboMenu.Add(new MenuCheckBox("combo.useM2", "PIERCEEE THAT ASS", true));
        ...
        private static void ComboMode()
        {
            ...
            var enemiesToTargetE = enemiesToTargetBase.Where(x => x.HasBuff("FrostDebuff"));
            ...
            var targetE = TargetSelector.GetTarget(enemiesToTargetE, targetMode, ERange);
            ...
            if (isCastingOrChanneling)
            {
                ...
                switch (LastAbilityFired)
                {
                    ...
                    case AbilitySlot.Ability2:
                        if (targetM2 != null)
                        {
                            var pred = TestPrediction.GetNormalLinePrediction(MyPos, targetM2, M2Range, M2Speed, M2Radius, false);
                            if (pred.CanHit)
                            {
                                LocalPlayer.Aim(pred.CastPosition);
                            }
            ...
            if (ComboMenu.GetBoolean("combo.useE") && MiscUtils.CanCast(AbilitySlot.Ability5))
            {
                ...
                {
                    var pred = TestPrediction.GetNormalLinePrediction(MyPos, targetE, ERange, ESpeed, ERadius, false);
                    if (pred.CanHit)
                    {
