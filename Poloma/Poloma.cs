...
namespace Poloma
{
	public class Poloma : IPlugin
	{
		...
		private void GameOnOnDraw(EventArgs args)
		{
			...
			if (DrawLmb)
				Drawing.DrawCircle(LocalPlayer.Instance.MapObject.Position, LmbSkill.Range, Color.red);

			if (DrawQ)
				Drawing.DrawCircle(LocalPlayer.Instance.MapObject.Position, QSkill.Range, Color.cyan);
			...
			if (DrawAim)
				Drawing.DrawCircle(LocalPlayer.AimPosition, 1f, Color.magenta);
		}
		...
		internal bool TryQ()
		{
			...
			var count = EntitiesManager.EnemyTeam?.Count(e =>
			{
				...
				{
					if (!casting)
						casting = e.AbilitySystem.IsCasting || e.IsChanneling || !e.HasBuff("BulwarkBuff");
				} else
				{
					casting = true;
				}
		...
		{
			var targetProj = EntitiesManager.EnemyTeam?.Where(x =>
				x.IsValid && !x.Living.IsDead && !x.IsCountering && !x.HasConsumeBuff &&
			...
			var target =
				targetProj.OrderBy(e => e.Living.Health)
		...
		internal bool CreateSkills()
		{
			try
			{
				LmbSkill = new SkillBase(AbilitySlot.Ability1, SkillType.Line, 8f, 15f, .25f);
