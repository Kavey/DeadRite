...
namespace PipRook
{
    public class PipRook : IAddon
    {
        ...
        private const float M2Range = 3.5f;
        private const float SpaceRange = 11.5f;
        private const float QRange = 8f;
        private const float ERange = 10f;
        private const float RRange = 3f;
        ...
        private const float SpaceSpeed = 20f;
        private const float M2AirTime = 0.55f;
        private const float QAirTime = 0.55f;
        private const float ESpeed = 22.5f;
        private const float EAirTime = 1f;
        ...
        private const float SpaceRadius = 0.8f;
        private const float QRadius = 2f;
        private const float ERadius = 3f;
        ...
        {
            LucieMenu = new Menu("pipluciemenu", "DaPip's Rook");
            ...
            ComboMenu.Add(new MenuCheckBox("combo.useM2", "Use CRUSHHH", true));
            ComboMenu.Add(new MenuSlider("combo.useM2.safeRange", "    ^ Safe range", 1.7f, 2.5f, 0f));
            ComboMenu.Add(new MenuCheckBox("combo.useSpace", "Use RUSHHH", true));
            ComboMenu.Add(new MenuSlider("combo.useSpace.safeRange", "    ^ Safe range", 1.7f, 2.5f, 0f));
            ...
            ComboMenu.Add(new MenuSlider("combo.useE.safeRange", "    ^ Safe range", 1.7f, 2.5f, 0f));
            ComboMenu.Add(new MenuCheckBox("combo.useEX1", "Use EX1 (Heal)", true));
            ComboMenu.Add(new MenuIntSlider("combo.useEX1.minEnergyBars", "    ^ Min energy bars", 3, 4, 1));
            ComboMenu.Add(new MenuCheckBox("combo.useEX2", "Use EX2 (Jambon)", true));
        ...
        private static void ComboMode()
        {
            ...
            {
                var safeRange = ComboMenu.GetSlider("combo.useM2.safeRange");
                if (LucieHero.EnemiesAroundAlive(safeRange) > 0)
                {
            ...
            if (!isCastingOrChanneling && ComboMenu.GetBoolean("combo.useSpace") && MiscUtils.CanCast(AbilitySlot.Ability3))
            {
            ...
            {
                var safeRange = ComboMenu.GetSlider("combo.useE.safeRange");
                if (LucieHero.EnemiesAroundAlive(safeRange) < 1)
                {
