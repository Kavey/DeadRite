...
namespace PipKaan
{
    public class PipKaan : IAddon
    {
        ...
        private static void HealTeammate()
        {
            ...
            var isCastingOrChanneling = KaanHero.AbilitySystem.IsCasting || KaanHero.IsChanneling || KaanHero.HasBuff("ElectricShield");
        ...
        private static void ComboMode()
        {
            ...
            if (isCastingOrChanneling)
            {
                ...
                switch (LastAbilityFired)
                {
                    ...
                    case AbilitySlot.Ability2:
                        ...
                        {
                            if (M2Target != null && !isCastingOrChanneling)
                            {
                    ...
                    case AbilitySlot.Ability1:
                        if (M1Target != null && !isCastingOrChanneling)
                        {
            ...
            if (!IsInUltimate)
            {
                ...
                if (ComboMenu.GetBoolean("combo.useE") && MiscUtils.CanCast(AbilitySlot.Ability5))
                {
                    ...
                    if (EnemyProjectileGoingToHitUnit(KaanHero, out closestProj))
                    {
                        if (LastAbilityFired == null && KaanHero.EnemiesAroundAlive(ERange) > 0)
                        {
                            {
        ...
        {
            KaanMenu = new Menu("pipfreyamenu", "DaPip's Freya", false);
